  ...
  <body>
        ...
        <div class="authentication-inner">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <!-- /Logo -->
              <h4 class="mb-2">Chào mừng đến với shop giày Ngọc Bích! 👋</h4>
              <p class="mb-4">Vui lòng đăng nhập vào tài khoản của bạn và bắt đầu trải nghiệm mua hàng</p>

              <form id="formAuthentication" class="mb-3" method="POST">
                <div class="mb-3">
                  <label for="email-sdt" class="form-label">Email or Số điện thoại</label>
                  <input
                    type="text"
                    class="form-control"
                    id="email-sdt"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loginForm.addEventListener('submit', function(event) {
            ...
            const formData = {
                'email-sdt': document.getElementById('email-sdt').value,
                'matkhau': document.getElementById('matkhau').value,
            };

            fetch('{{ url("http://localhost/shoe-api/public/api/khachhang/dangnhap") }}', {
                method: 'POST',
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    // If the server responds with a non-OK HTTP status, throw the response to handle errors
                    return response.json().then(json => Promise.reject(json));
                }
                return response.json();
            })
            .then(data => {
                // On successful login, show success toast and potentially redirect the user
                showToast('Thành công', data.message, 'bg-success');
            ...
            .catch(error => {
                // Process and show errors using the showToast function
                if (error.errors) {
                    Object.keys(error.errors).forEach(key => {
                        showToast('Lỗi', error.errors[key], 'bg-danger');
                    });
                } else {
                    // Use a generic error message if specific errors are not provided
                    showToast('Thất bại', error.message || 'Login failed. Please check your credentials.', 'bg-danger');
                }
            });
        });
    });
