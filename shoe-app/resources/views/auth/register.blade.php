            ...
            <div class="card-body">
              ...
              <div class="app-brand justify-content-center">
                <a href="index.html" class="app-brand-link gap-2">
                  ...
                  <span class="app-brand-text demo text-body fw-bolder">Giày</span>
                </a>
              </div>
              <!-- /Logo -->
              <h4 class="mb-2">Cuộc trải nghiệm bắt đầu 🚀</h4>
                ...
                <div class="mb-3">
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="terms-conditions">
                      Tôi đồng ý với
                      <a href="javascript:void(0);">điều khoản và dịch vụ</a>
                    </label>
                  </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', function (event) {
                ...
                if (!ten.trim()) {
                    showToast('Lỗi', 'Tên không được để trống', 'bg-warning');
                    return;
                }

                // Kiểm tra email đúng định dạng
                if (!email.match(/^\S+@\S+\.\S+$/)) {
                    showToast('Lỗi', 'Email không hợp lệ', 'bg-warning');
                    return;
                }
                ...
                if (!sdt.match(/^\d{10,11}$/)) {
                    showToast('Lỗi', 'Số điện thoại không hợp lệ', 'bg-warning');
                    return;
                }

                // Kiểm tra địa chỉ không được để trống
                if (!diachi.trim()) {
                    showToast('Lỗi', 'Địa chỉ không được để trống', 'bg-warning');
                    return;
                }

                // Kiểm tra mật khẩu
                if (matkhau.length < 6) {
                    showToast('Lỗi', 'Mật khẩu phải có ít nhất 6 ký tự', 'bg-warning');
                    return;
                }

                // Kiểm tra mật khẩu và nhập lại mật khẩu
                if (matkhau !== repassword) {
                    showToast('Lỗi', 'Mật khẩu nhập lại không khớp', 'bg-warning');
                    return;
                }

                // Kiểm tra điều khoản đã được chấp nhận
                if (!terms) {
                    showToast('Lỗi', 'Bạn phải đồng ý với điều khoản sử dụng', 'bg-warning');
                    return;
                }

                // Gửi dữ liệu nếu tất cả thông tin hợp lệ
                sendData({
                    email: email,
                    ten: ten,
                    sdt: sdt,
            ...
            function sendData(data) {
                fetch('  http://localhost/shoe-api/public/api/khachhang/dangky', {
                    method: 'POST',
                    headers: {
                ...
                .then(result => {
                    showToast('Thành công', 'Đăng ký thành công!', 'bg-success');
                ...
                .catch(error => {
                    ...
                    // Check if the error object has the `errors` attribute, which contains validation messages
                    if (error.errors) {
                        // Loop through the keys of the errors object to display all validation errors
                        Object.keys(error.errors).forEach(key => {
                            showToast('Thất bại', error.errors[key], 'bg-warning');
                        });
                    } else {
                        // Use a generic error message if the format isn't as expected
                        showToast('Thất bại', 'Lỗi hệ thống. Vui lòng thử lại sau.', 'bg-danger');
                    }
                });
            }
